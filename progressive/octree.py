...
from progressive.util import intersection
from progressive.weights import weigh_antimatter, weigh_contrib
...
def traverse_for_indices(node, p, indices, depth, weight_cb, frustum=None):
    if isinstance(node, o3d.geometry.OctreeLeafNode):
        ...
        if frustum is not None:
            node_indices_t = intersection(node_indices_t, frustum)
        ...
        mask = torch.ones_like(node_indices_t, dtype=torch.bool, device='cuda:0')
        mask[largest_k_indices_main] = False
        filtered_t = node_indices_t[mask]
